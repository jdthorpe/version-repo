...
import {   sync_repository, deferred_repository, resource_data, repository } from   "../src/typings";
...
export function generate_tests(inst:test_instance):void{
    ...
    describe("<General Tests> " + inst.name, function() {
        ...
        it('POST(create) should require a version',function() {
            ...
            try{
                request = Promise.resolve(inst.repo.create(<resource_data<any>>{name:'foo'}));
            }
            catch(err){
        ...
        it('versions(name) return an array',function() {
            // FOR TESTING THE READ ONLY BUFFER
            return Promise.resolve(inst.repo.versions('bar')).then(x => {
        ...
        it('versions() return a dictionary',function() {
            // FOR TESTING THE READ ONLY BUFFER
            return Promise.resolve(inst.repo.versions()).then(x => {
                (typeof x).should.equal('object');
        ...
        it('PUT(updtate) should require a version',function() {
            ...
            var request:Promise<any>;
            try{
                request = Promise.resolve(inst.repo.update(<resource_data<any>>{name:'foo'}));
            }
            catch(err){
                request = Promise.reject(err);
            }
        ...
        it('GET "/" (`packages()`) should return the list of libraries',function() {
            return Promise.resolve(inst.repo.packages()).then(x => x.sort())
