...
var repo = new memory_repo_1.MemoryRepo();
...
describe('multiple version resolution', function () {
    it('should handle single packages with *no* dependencies.', function () {
        return version_resolution_1.calculate_dependencies([{ name: 'a', version: '1.1.2' }], repo)
            .should.eventually.deep.equal([{ name: 'a', version: '1.1.2' }]);
    });
    it('should handle single packages with *no* dependencies.', function () {
        return version_resolution_1.calculate_dependencies([{ name: 'a', version: '~1.1.1' }], repo)
            .should.eventually.deep.equal([{ name: 'a', version: '1.1.3' }]);
    });
    it('should handle single packages with *no* dependencies.', function () {
        version_resolution_1.calculate_dependencies([{ name: 'a', version: '~1.x' }], repo)
            .should.eventually.deep.equal([{ name: 'a', version: '1.2.3' }]);
    });
    it('should handle single packages with dependencies.', function () {
        return version_resolution_1.calculate_dependencies([{ name: 'b', version: '1.0.0' }], repo)
            .should.eventually.deep.equal([{ name: 'b', version: "1.0.0" }, { name: 'a', version: '1.0.0' }]);
    });
    it('should handle single packages with dependencies.', function () {
        return version_resolution_1.calculate_dependencies([{ name: 'b', version: '1.1.1' }], repo)
            .should.eventually.deep.equal([{ name: 'b', version: "1.1.1" }, { name: 'a', version: '1.1.3' }]);
    });
    it('should handle single packages with dependencies.', function () {
        return version_resolution_1.calculate_dependencies([{ name: 'b', version: '1.1.3' }], repo)
            .should.eventually.deep.equal([{ name: 'b', version: "1.1.3" }, { name: 'a', version: '1.1.2' }]);
    });
    it('should handle single packages with dependencies.', function () {
        return version_resolution_1.calculate_dependencies([{ name: 'b', version: '1.1.4' }], repo)
    ...
    it('should error out on internlly inconsietent specifications .', function () {
        return version_resolution_1.calculate_dependencies([{ name: 'c', version: '1.1.3' }], repo)
            .should.be.rejectedWith(/Version Conflict:/);
    });
    it('should error out with inconsietent arrays .', function () {
        return version_resolution_1.calculate_dependencies([{ name: 'a', version: '~1.1.1' }, { name: 'c', version: '~1.1.3' }], repo)
            .should.be.rejectedWith(/Version Conflict:/);
    });
    it('should error out with inconsietent arrays .', function () {
        return version_resolution_1.calculate_dependencies([{ name: 'a', version: '~1.1.1' }, { name: 'b', version: '~1.1.4' }], repo)
            .should.be.rejectedWith(/Version Conflict:/);
    });
    it('should reject confliced dependencies (previously accomodated both).', function () {
        return version_resolution_1.calculate_dependencies([{ name: 'a', version: '~1.1.1' }, { name: 'b', version: '~1.1.3' }], repo)
            .should.be.rejectedWith(/Version Conflict:/);
        ;
    });
});
